...
router.get('/check-exists', (req, res) => {
    ...
    // Llama al nuevo SP que creamos para accesorios
    const sql = `CALL \`${dbConfig.database}\`.\`sp_CheckSuperCategoriaAccesorioActivaExists\`(?, ?, ?);`;

    Basedatos.query(sql, [fab, cat, sub], (err, results) => {
        if (err) {
            console.error('Error al verificar supercategoría de accesorio:', err);
