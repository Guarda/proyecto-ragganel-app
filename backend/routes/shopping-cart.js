...
router.get('/vigentes/:idUsuario', (req, res) => {
  ...
  const query = 'CALL sp_ConsultarCarritosVigentes(?)';
...
router.delete('/:idCarrito', (req, res) => {
  ...
  const query = 'CALL sp_LiberarCarrito(?)';

  db.query(query, [idCarrito], (err, result) => {
    if (err) {
...
router.get('/listar-carrito-en-curso-cliente-usuario', (req, res) => {
    ...
    const query = 'CALL sp_ObtenerContenidoCarrito(?, ?)';
    db.query(query, [IdUsuario, IdCliente], (err, results) => {
        if (err) {
